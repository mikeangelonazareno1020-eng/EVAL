...
class Branch
{
    ...
    public function create()
    {
        ...
        if ($stmt->execute()) {
            $this->branch_id = $this->conn->lastInsertId();
            return true;
        }
    ...
    public function readPaginated($page = 1, $records_per_page = 10, $search = '')
    {
        ...
        $query = "SELECT branch_id, branch_name, date_entry
                  FROM " . $this->table_name . "
                  WHERE 1=1";

        if (!empty($search)) {
            $query .= " AND (branch_name LIKE :search)";
        }
    ...
    {
        $query = "SELECT branch_id, branch_name, date_entry
                  FROM " . $this->table_name . " WHERE branch_id = :branch_id LIMIT 1";
